...
public sealed class GetReportsQueryHandler : IRequestHandler<GetReportsQuery, AppResult<ReportsDto>>
{
    ...
    public async Task<AppResult<ReportsDto>> Handle(GetReportsQuery request, CancellationToken ct)
    {
        ...
        var propertyExists = await _db.Properties.AsNoTracking()
            .AnyAsync(p => p.Id == request.PropertyId && (p.Account.OwnerUserId == _current.UserId || p.Account.Users.Any(au => au.UserId == _current.UserId && au.IsActive)), ct);
