...
public sealed class GetOperationalTasksQueryHandler : IRequestHandler<GetOperationalTasksQuery, AppResult<List<OperationalTaskDto>>>
{
    ...
    public GetOperationalTasksQueryHandler(IAppDbContext db, ICurrentUser current, IPropertyFeatureService features, IUserAccessService access)
    {
        _db = db;
        _current = current;
        _features = features;
    ...
    public async Task<AppResult<List<OperationalTaskDto>>> Handle(GetOperationalTasksQuery request, CancellationToken ct)
    {
        ...
        if (!await _access.HasPropertyModuleAccessAsync(request.PropertyId, SaasModule.Housekeeping, ct))
            return AppResult<List<OperationalTaskDto>>.Fail("forbidden", "No tenés acceso al módulo de housekeeping.");
